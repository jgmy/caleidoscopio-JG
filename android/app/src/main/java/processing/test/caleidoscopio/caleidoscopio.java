package processing.test.caleidoscopio;
...
//float absurd=0.01*((frameCount %300)-150);
     ...
     */
      for (int f=0;f<6;f++){
        ...
        if (shapeType==HEXSTAR||shapeType==MEGAHEX){
           ...
           translate(0,-1.5f*imagen.height);
           image(imagen,0,0);
      ...
      if (shapeType==MEGAHEX){
        for (int f=0;f<6;f++){
         ...
         translate(sin(PI/3)*1.5f*imagen.height,-0.25f*imagen.height);
         image(imagen,0,0);
         popMatrix();
         pushMatrix();
         translate(-sin(PI/3)*1.5f*imagen.height,-0.25f*imagen.height);
 ...
 public void makemask(PGraphics img){
 int wi=img.width;
 int he=img.height;
  float cx=wi/2;
  float cy=he/2;
     ...
     */
      for (int f=0;f<6;f++){
        ...
        if (shapeType==HEXSTAR||shapeType==MEGAHEX){
           /* and now another */
           /* same angle, opposite side*/
           Pho.translate(0,-1.5f*imagen.height);
           Pho.image(imagen,0,0);
      ...
      if (shapeType==MEGAHEX){
        for (int f=0;f<6;f++){
          ...
          Pho.pushMatrix();
          /* sin( pi/3 ) to get apothem */
          Pho.translate(sin(PI/3)*1.5f*imagen.height,-0.25f*imagen.height);
          Pho.image(imagen,0,0);
          Pho.popMatrix();
          Pho.pushMatrix();
          Pho.translate(-sin(PI/3)*1.5f*imagen.height,-0.25f*imagen.height);
 ...
 * write permission
 */
public void manageWritePermission(boolean granted){
  if(!granted)
    saveWord="CAN'T SAVE";
  else
    saveCapture();
...
public void mousePressed()
{
  ...
  if (mouseX<rd){
    for (int f=1;f<4;f++){
      if (dist(mouseX,mouseY,rd,PApplet.parseFloat(f)*height/4)<rd){
        shapeType=f;
        break;
      }
    }
...
public void drawShapeIcon(){
  ...
  for (float f=0;f<6;f++){
    ...
    line(
      cx+rd*cos((f+0.5f)*PI/3)/2,cy+rd*sin((f+0.5f)*PI/3)/2,
      cx+rd*cos((f+3.5f)*PI/3)/2,cy+rd*sin((f+3.5f)*PI/3)/2
    );
  }
  ...
  for (float f=0;f<6;f++){
    line(
      cx+c3*cos((f+0.5f)*PI/3),cy+c3*sin((f+0.5f)*PI/3),
      cx+c3*cos((f+1.5f)*PI/3),cy+c3*sin((f+1.5f)*PI/3)
    );
    line(
      cx+c3*cos((f+0.5f)*PI/3),cy+c3*sin((f+0.5f)*PI/3),
      cx+c3*cos((f+3.5f)*PI/3),cy+c3*sin((f+3.5f)*PI/3)
    );
    line(
      cx+rd*cos(f*PI/3),cy+rd*sin(f*PI/3),
      cx+rd*cos((f+2)*PI/3),cy+rd*sin((f+2)*PI/3)
